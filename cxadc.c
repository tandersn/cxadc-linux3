...
static int cxadc_char_open(struct inode *inode, struct file *file)
{
	...
	cx_write(MO_AGC_GAIN_ADJ4, (sixdb<<23)|(0<<22)|(vga_en<<21)|(level<<16)|(0xff<<8)|(0x0<<0));
			     ...
			     unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == 0x12345670) {
		...
		/* control gain also bit 16 */
		cx_write(MO_AGC_GAIN_ADJ4, (sixdb<<23)|(0<<22)|(vga_en<<21)|(gain<<16)|(0xff<<8)|(0x0<<0));
	}
